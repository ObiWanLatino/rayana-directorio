...
export async function POST(request: Request) {
  try {
    ...
    const path = `${supplierId}/cover.${ext}`;
    ...
    if (upErr) {
      console.error("[POST /api/admin/upload-cover] upload error:", upErr);
    ...
    const { error: updErr } = await admin
      .from("suppliers")
      .update({ cover_url: versionedUrl, updated_at: now })
      .eq("id", supplierId);

    if (updErr) {
      console.error("[POST /api/admin/upload-cover] db error:", updErr);
      return NextResponse.json({ error: updErr.message }, { status: 400 });
    }

    return NextResponse.json({ cover_url: versionedUrl });
  } catch (error) {
    console.error("[POST /api/admin/upload-cover] unhandled:", error);
    return NextResponse.json(
