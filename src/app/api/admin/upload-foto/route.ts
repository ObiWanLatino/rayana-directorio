...
export async function POST(request: Request) {
  try {
    ...
    if (upErr) {
      console.error("[POST /api/admin/upload-foto] upload error:", upErr);
    ...
    if (updErr) {
      console.error("[POST /api/admin/upload-foto] db error:", updErr);
      return NextResponse.json({ error: updErr.message }, { status: 400 });
    }

    return NextResponse.json({ url: versionedUrl });
  } catch (error) {
    console.error("[POST /api/admin/upload-foto] unhandled:", error);
    return NextResponse.json(
