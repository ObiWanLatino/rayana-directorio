...
export async function POST(request: Request) {
  try {
    ...
    if (upErr) {
      console.error("[POST /api/admin/upload-logo] upload error:", upErr);
    ...
    if (updErr) {
      console.error("[POST /api/admin/upload-logo] db error:", updErr);
      return NextResponse.json({ error: updErr.message }, { status: 400 });
    }

    return NextResponse.json({ logo_url: versionedUrl });
  } catch (error) {
    console.error("[POST /api/admin/upload-logo] unhandled:", error);
    return NextResponse.json(
